...
class TestCreateTodo:
    ...
    async def test_create_todo_with_valid_title(self, async_client: httpx.AsyncClient):
        """Creating a todo with a valid title returns 201."""
        response = await async_client.post("/api/todos", json={"title": "Buy milk"})
    ...
    async def test_create_todo_rejects_overlength_title(
        ...
        long_title = "a" * 201
        response = await async_client.post("/api/todos", json={"title": long_title})
    ...
    async def test_create_todo_accepts_max_length_title(
        ...
        title_200 = "a" * 200
        response = await async_client.post("/api/todos", json={"title": title_200})

        assert response.status_code == 201
        assert response.json()["title"] == title_200

    async def test_create_todo_trims_whitespace(self, async_client: httpx.AsyncClient):
        """Leading/trailing whitespace is trimmed from the title."""
        response = await async_client.post("/api/todos", json={"title": "  Buy eggs  "})
...
class TestListTodos:
    ...
    async def test_list_todos_empty(self, async_client: httpx.AsyncClient):
    ...
    async def test_list_todos_after_creation(self, async_client: httpx.AsyncClient):
        """After creating todos they appear in the list."""
        await async_client.post("/api/todos", json={"title": "First"})
        await async_client.post("/api/todos", json={"title": "Second"})
    ...
    async def test_list_todos_response_structure(self, async_client: httpx.AsyncClient):
        """Each item in the list has the full TodoResponse shape."""
        await async_client.post("/api/todos", json={"title": "Check structure"})
    ...
    async def test_list_todos_filter_active(self, async_client: httpx.AsyncClient):
        """Filtering by status=active returns only active todos."""
        await async_client.post("/api/todos", json={"title": "Active todo"})
        resp2 = await async_client.post("/api/todos", json={"title": "Done todo"})

        # Mark the second as completed
        todo_id = resp2.json()["id"]
        await async_client.patch(f"/api/todos/{todo_id}", json={"completed": True})

        response = await async_client.get("/api/todos", params={"status": "active"})
    ...
    async def test_list_todos_filter_completed(self, async_client: httpx.AsyncClient):
        ...
        response = await async_client.get("/api/todos", params={"status": "completed"})
    ...
    async def test_list_todos_filter_all(self, async_client: httpx.AsyncClient):
        """Filtering by status=all returns all todos."""
        await async_client.post("/api/todos", json={"title": "Todo A"})
        await async_client.post("/api/todos", json={"title": "Todo B"})

        response = await async_client.get("/api/todos", params={"status": "all"})
...
class TestUpdateTodo:
    ...
    async def test_update_title(self, async_client: httpx.AsyncClient):
        """Updating the title returns 200 with the new title."""
        create_resp = await async_client.post("/api/todos", json={"title": "Old title"})
    ...
    async def test_update_completed(self, async_client: httpx.AsyncClient):
        """Updating completed to true returns 200."""
        create_resp = await async_client.post("/api/todos", json={"title": "Mark done"})
    ...
    async def test_update_nonexistent_todo(self, async_client: httpx.AsyncClient):
        """Updating a non-existent todo returns 404."""
        response = await async_client.patch("/api/todos/99999", json={"title": "Nope"})

        assert response.status_code == 404
        assert response.json() == {"detail": "TODO not found"}

    async def test_update_preserves_other_fields(self, async_client: httpx.AsyncClient):
...
class TestDeleteTodo:
    ...
    async def test_delete_existing_todo(self, async_client: httpx.AsyncClient):
        """Deleting an existing todo returns 204 No Content."""
        create_resp = await async_client.post("/api/todos", json={"title": "To delete"})
        todo_id = create_resp.json()["id"]

        delete_resp = await async_client.delete(f"/api/todos/{todo_id}")

        assert delete_resp.status_code == 204

    async def test_delete_nonexistent_todo(self, async_client: httpx.AsyncClient):
    ...
    async def test_deleted_todo_not_in_list(self, async_client: httpx.AsyncClient):
        """After deletion the todo no longer appears in the list."""
        create_resp = await async_client.post("/api/todos", json={"title": "Gone soon"})
        todo_id = create_resp.json()["id"]
